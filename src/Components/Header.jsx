...
import { setLogout } from "../ReduxSystem/Slices/loginSlice";
...
const Header = ({ cartProducts, handleChangeMode }) => {
  ...
  const tk = userToken;
  // console.log(tk);
  const userData = tk ? jwtDecode(tk) : null;
    ...
    <div className="px-7 py-3 rounded-none bg-green-900 w-full  dark:bg-[#1d2029] bg-opacity-1">
      ...
      {userToken ? (
        <div>
          <Menu>
            ...
            <MenuList>
              ...
              <MenuItem className="flex items-center gap-2">
                ...
                <Typography onClick={()=>navigate(`/editprofile/${userData.id}`)} variant="small" className="font-medium">
                  Edit Profile
                </Typography>
              </MenuItem>
          ...
          <div className="dark:text-white">hello {userInfo?.fullName}</div>
        </div>
      ) : (
        <div>
          <Link to={`/signup`} className="hover:text-green-500 mr-2">
            Reestration
          </Link>
          /
          <Link to={`/login`} className="hover:text-green-500 ml-2">
            Login
          </Link>
        </div>
      )}
