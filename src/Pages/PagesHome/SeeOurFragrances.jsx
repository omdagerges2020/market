...
import { getFragrancesProducts } from "../../ReduxSystem/Slices/fragrancesSlice";
import PageNotFound from './../../PageNotFound';
import { Link } from "react-router-dom";

const SeeOurFragrances = () => {
  const { fragrances, loadingFragrances, errorFragrances } = useSelector(
    (state) => state.fragrancesProducts
  );
  //   console.log(laptops);
  ...
  // UseEffect
  useEffect(() => {
    dispatch(getFragrancesProducts());
  }, []);

  // pending condition
  if (loadingFragrances) {
    return <Loading />;
  }
  ...
  return (
    <div className="mt-9 container mx-auto z-0 px-5 lg:px-0 xl:px-0">
